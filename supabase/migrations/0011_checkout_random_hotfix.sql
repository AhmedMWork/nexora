...
--
-- Cause:
--   The atomic order RPC runs with `set search_path = public`.
--   In Supabase, pgcrypto functions can live outside `public`, so
--   an unqualified pgcrypto random-byte call may not resolve inside the RPC.
--
-- Fix:
--   Replaces the order-number random suffix with md5(clock_timestamp + random),
