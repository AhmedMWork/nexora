-- NEXORA — Admin order editing support
...
create table if not exists public.order_followup_types (
...
create unique index if not exists order_followup_types_type_key
on public.order_followup_types(type);

alter table public.order_followup_types enable row level security;

do $$
begin
  if not exists (
    select 1 from pg_policies where schemaname = 'public' and tablename = 'order_followup_types' and policyname = 'order_followup_types_service_role_all'
  ) then
    create policy "order_followup_types_service_role_all"
      on public.order_followup_types for all
      using (auth.role() = 'service_role')
      with check (auth.role() = 'service_role');
  end if;
end $$;
