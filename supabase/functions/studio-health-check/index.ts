...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: orderRpc, error: rpcError } = await supabase.rpc('nexora_rate_limit', { bucket_key_value: 'health-check', limit_value: 99999, window_seconds: 60 });
    checks.push(rpcError ? { key: 'rpc:rate_limit', label: 'DB rate-limit RPC', status: 'fail', message: rpcError.message, fix: 'Run supabase db push for stable admin migration.' } : { key: 'rpc:rate_limit', label: 'DB rate-limit RPC', status: 'ok', message: `Rate limiter is ready (${orderRpc?.remaining ?? 'ok'} remaining in health bucket).` });

    const { data: diagnostics, error: diagnosticsError } = await supabase.rpc('nexora_diagnostics');
    const diag: any = diagnostics || {};
    if (diagnosticsError) {
      checks.push({ key: 'checkout:diagnostics', label: 'Checkout diagnostics RPC', status: 'fail', message: diagnosticsError.message, fix: 'Run supabase db push for stability migration.' });
    } else {
      ...
      checks.push({ key: 'checkout:shipping-rpc', label: 'Shipping RPC', status: diag.shippingRpcReady ? 'ok' : 'fail', message: diag.shippingRpcReady ? 'Delivery calculation RPC is ready.' : 'Shipping calculation RPC is missing.', fix: diag.shippingRpcReady ? undefined : 'Run supabase db push for shipping hardening/stability migrations.' });
